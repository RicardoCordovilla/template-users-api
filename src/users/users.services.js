...
const patchUser = (req, res) => {
    ...
    usersControllers.updateUser(id, { firstName, lastName, phone, gender, country, role })
        .then(data => {
            if (data[0]) {
...
const deleteMyUser=(req,res)=>{
    const id=req.user.id
    usersControllers.updateUser(id,{status:'off'})
    .then(()=>res.status(204).json({message:`User with id: ${id}, was deleted`}))
}
